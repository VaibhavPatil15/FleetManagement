package frameworkUtils;

public interface reusableComponent {

	public static String browserType = "chrome";
	public static String chrome_exte = "webdriver.chrome.driver";
	public static String edge_exte = "webdriver.edge.driver";
	public static String chrome_path = "C:\\Users\\Vp671\\git\\FleetManagemnet\\FleetManagement\\browser\\chromedriver.exe";
	public static String edge_path = "D:\\JAVA\\ReferMeGroup\\JAVA\\batch50-main\\batch50-main\\FleetManagemnet\\browser\\chromedriver.exe";
	public static String url = "https://demo.guru99.com/test/newtours/register.php";
	public static String alertUrl="https://demo.guru99.com/test/delete_customer.php";
	public static String waiturl = "https://www.hyrtutorials.com/p/waits-demo.html#google_vignette";
	public static String tabledataurl ="https://demo.guru99.com/test/web-table-element.php";
	public static String propertiesFile = "C:\\Users\\Vp671\\git\\FleetManagemnet\\FleetManagement\\OR\\objectRepositories";
	public static String multiWindows="https://demoqa.com/browser-windows";
	public static String calender="https://demo.guru99.com/test/";
	public static String red="https://www.redbus.in/";
	public static String uploadUrl="https://www.tutorialspoint.com/selenium/practice/upload-download.php";
	public static String check="https://demo.guru99.com/test/radio.html";
	public static String excelFilePath = "C:\\Users\\Vp671\\git\\FleetManagemnet\\FleetManagement\\src\\test\\resources\\TestData.xlsx";
}


//https://www.tutorialspoint.com/selenium/practice/upload-download.php